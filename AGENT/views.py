...
def filter_profil(request):
	...
	if request.method == 'POST':
		...
		o = User_AGENTIS.objects.get(login = request.session['member_id'])
		mess = display_mess(o)
		is_mess = mess.is_mes()
		for offers in p:
			offers.img.img = offers.img.goodname()
		return render_to_response('AGENTIS/profile.html',{'p': o, 'm': mess.nm, 'who': o.who, 'ps': p, 'is_mess': is_mess}, RequestContext(request))

def filter_spec(request):
	c = {}
	c.update(csrf(request))
	errors = []
	if request.method == 'POST':
		...
		if POST['spec'] == '1':
			...
			is_mess = mess.is_mes()
			return render_to_response('AGENTIS/profile.html',{'errors': errors[0],'p': p, 'm': mess.nm, 'who': p.who, 'ps': p, 'is_mess': is_mess}, RequestContext(request))
		p = User_AGENTIS.objects.filter(specialization = POST['spec'], who = 1)
		o = User_AGENTIS.objects.get(login = request.session['member_id'])
		mess = display_mess(o)
		is_mess = mess.is_mes()
		for offers in p:
...
def registration_result(request):
	...
	if request.method == 'POST':
		...
		if request.POST['region'] == 's1':
			errors.append('Введите вашу специализацию')
